...
def verify_pass(saved, received):
    ...
    The only supported formats are unix crypt(3) and sha1 and Also Plain USER:PASS.
    """
    if saved.startswith("{SHA}"):
        import base64
        import hashlib
...
class HTTPBackendHandler(CGIHTTPRequestHandler, object):
    ...
    def _send_response_with_header(self, code, message,data):
        """Send header with body"""
        self.send_response(code, message)
        self.end_headers()
    ...
    def maybe_list_repos(self):
        """List repos if non-cgi GET seems legit
        ...
        def bail():
            if os.path.exists(abspath):
                ...
                msg = (HTTPStatus.NOT_FOUND, "Namespace does not exist")
            self.send_error(*msg)
        ...
        if "form" in contype:
            ...
            if "urlencoded" in contype:
                try:
                    ...
                    from urllib import unquote_plus
                request_body = unquote_plus(request_body.decode())
            if "list=1" not in request_body:
                bail()
                return False

        try:
            _stdout = list_repos(abspath)
        except Exception:
            self.log_exception("E: Problem Listing repos")
            self.send_error(
                HTTPStatus.INTERNAL_SERVER_ERROR, "Problem Listing repos"
            )
            return False
    ...
    def parse_request(self):
        ...
        if not is_ghb_bound(self.command, self.path):
            if self.command == "POST":
                ...
                elif not self.path.rstrip("/").endswith(".git"):
                        self.maybe_list_repos()
                else:
...
def _validate_logpath(path):
    ...
    It will Reset the Logs every Run
    """
